#!/usr/bin/env python3
"""A4-live instruments: publish Governor probe checks through the full
...
EVIDENCE_SCHEMA = "EnforcementProbeEvidence-v1"
...
def publish(args):
    ...
    try:
        ...
        epoch_id = f"a4live-{args.head[:12]}"
        ...
        decision_id = history.record(
            ...
            decision_rule_revision="a4live.1", auth_generation=3,
...
def main():
    ...
    ap.add_argument("command", choices=["publish", "state"])
    ...
    ap.add_argument("--db", default=".captures/a4live/decisions.sqlite3")
    ap.add_argument("--out", default=None)
    args = ap.parse_args()
    result = {"publish": publish, "state": state}[args.command](args)
